...
public class Picture extends SimplePicture 
{
  ...
  public void fixUnderwater()
  {
      ...
      for (Pixel[] rowArray : pixels)
      {
          for (Pixel pixelObj : rowArray)
          {
              pixelObj.setGreen(pixelObj.getGreen()+pixelObj.getBlue()/3);
              pixelObj.setRed(255-pixelObj.getRed());
              pixelObj.setBlue(pixelObj.getGreen()+pixelObj.getBlue()/3);
    }
}
}
                 ...
                 int startRow,int endRow, int startCol, int endCol)
  {
    ...
    Pixel[][] toPixels = this.getPixels2D();
    Pixel[][] fromPixels = fromPic.getPixels2D();
    for (int fromRow = startRow; 
         fromRow < fromPixels.length;
         fromRow++)
    {
      for (int fromCol = startCol; 
           fromCol < fromPixels[0].length;  
           fromCol++)
      {
        fromPixel = fromPixels[fromRow][fromCol];
        toPixel = toPixels[endRow][endCol];
        toPixel.setColor(fromPixel.getColor());
      }
    }   
  }
  ...
  public void myCollage()
  {
    ...
    this.copy(flower2,100,500, 0, 200);
    this.copy(flower1,200,0);
    this.copy(flower1,400,0);
    this.copy(flower2,500,0);
    this.mirrorDiagonal();
